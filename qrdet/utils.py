...
def draw_detections(image, boxes, scores, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    for box, score, class_id in zip(boxes, scores, class_ids):
        ...
        # Draw rectangle
        cv.rectangle(mask_img, (x1, y1), (x2, y2), color, 2)

        label = class_names[class_id]
        caption = f'{label} {int(score * 100)}%'
        (tw, th), _ = cv.getTextSize(text=caption, fontFace=cv2.FONT_HERSHEY_SIMPLEX,
        ...
        cv.putText(mask_img, caption, (x1, y1),
                    cv.FONT_HERSHEY_SIMPLEX, size, (255, 255, 255), text_thickness, cv2.LINE_AA)
...
def draw_masks(image, boxes, class_ids, mask_alpha=0.3, mask_maps=None):
    ...
    return cv.addWeighted(mask_img, mask_alpha, image, 1 - mask_alpha, 0)


def draw_comparison(img1, img2, name1, name2, fontsize=2.6, text_thickness=3):
    (tw, th), _ = cv.getTextSize(text=name1, fontFace=cv2.FONT_HERSHEY_DUPLEX,
    ...
    (tw, th), _ = cv.getTextSize(text=name2, fontFace=cv2.FONT_HERSHEY_DUPLEX,
                                  fontScale=fontsize, thickness=text_thickness)
    x1 = img2.shape[1] // 3
    y1 = th
    offset = th // 5
    cv.rectangle(img2, (x1 - offset * 2, y1 + offset),
                  (x1 + tw + offset * 2, y1 - th - offset), (94, 23, 235), -1)

    cv.putText(img2, name2,
                (x1, y1),
                cv2.FONT_HERSHEY_DUPLEX, fontsize,
                (255, 255, 255), text_thickness)
    ...
    if combined_img.shape[1] > 3840:
        combined_img = cv.resize(combined_img, (3840, 2160))
